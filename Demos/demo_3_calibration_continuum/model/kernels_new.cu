...
__global__ void kernelGrowA(double *a_new, double *a_old, double *dt, double S, double KA)
{
	...
	for(int i=0; i<ZMAX; i++)
  	{
		...
		a_new[out_idx] = sa[ty][tx] + dtS*sa[ty][tx]*(1.0-sa[ty][tx]) - dtKA*sa[ty][tx];;
	}
}


// Growth kernel for species A (cancer cells)
__global__ void kernelAleeA(double *a_new, double *a_old, double *a_first, double *dt, double S, double KA, int iter)
{
    ...
    for(int i=0; i<ZMAX; i++)
    {
        ...
        a_new[out_idx] = sa[ty][tx] + dtS*sa[ty][tx]*(1.0  -  1.028*sf[ty][tx]/(sa[ty][tx] + 942.8*time)) - dtKA*sa[ty][tx];
    }
}
