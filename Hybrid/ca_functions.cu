...
void initialize_ca(DataArray& host, CAArray& casp, vector<Cell>& cells) {
    ...
    for (int i=0; i<3*zsize; i++) {casp.lattice[i]=true; casp.phen[i]=true;};//filling base
    for (int i=zsize*(Nz-3); i<total; i++) {casp.lattice[i]=true;};//filling top
    for (int i=0; i<3*ysize; i++)
    {
        for (int j=i; j<=zsize*(Nz-3)+i; j += zsize) {casp.lattice[j]=true;}; //filling left boundary perpendicular to y-axis
    }
    for (int i=0; i<=zsize-3*ysize; i+=ysize)
    {
        for (int j=i; j<=zsize*(Nz-3)+i; j += zsize) {casp.lattice[j]=true;}; //filling back boundary perpendicular to x-axis
    }
    for (int i=ysize-3; i< ysize   ; i+=ysize)
    {
        for (int j=i; j<=zsize*(Nz-3)+i; j += zsize) {casp.lattice[j]=true;}; //filling front boundary perpendicular to x-axis
    }
    for (int i=zsize-3*ysize; i<zsize; i++)
    {
        for(int j=i; j< total ; j += zsize) {casp.lattice[j]=true;}; //filling right boundary perpendicular to y-axis
    }


	casp.indcNeigh    = {-3,3,-3*ysize,3*ysize,-3*zsize,3*zsize}; //neighborhood

    random_device rand_dev;
    mt19937 generator(rand_dev());

	char filename[4096];
    sprintf(filename, "IC/coord_idx_%s_D5.txt", host.dat);
...
int returnPosChance(CAArray& casp, DataArray& host,InitialData& id, int indx) {
    ...
    int neigh = 0;
	int scl   = 3; //scale parameter accounts for the size of the cell
...
*/
	...
	if(indx <= 6*zsize && casp.phenchange <= 0){
        ...
        else if(!casp.lattice[indx+casp.indcNeigh[3]] && randmov>=normRi3D[3] && randmov<normRi3D[4])                	
				neigh = indx+casp.indcNeigh[3]; //printf("Right \n");
        
        else if(!casp.lattice[indx+casp.indcNeigh[4]] && randmov>=normRi3D[4] && randmov<=normRi3D[5])                	
				neigh = indx+casp.indcNeigh[4]; //printf("Down \n");
        
        else if(!casp.lattice[indx+casp.indcNeigh[5]] && randmov>=normRi3D[5] && randmov<=normRi3D[6])                	
				neigh = indx+casp.indcNeigh[5]; //printf("Up \n");
	}
	return neigh;
...
void caStep(vector<Cell>& cells, vector<Cell>& cellsTmp, CAArray& casp, DataArray& host, InitialData& id)
{
    ...
    int nPhen, nnPhen, newSite, newPos;
	double Ae, d_prob; //local cell density, spontaneous death probability
    ...
    random_shuffle(cells.begin(), cells.end()); //shuffling cells
  	...
  	while (!cells.empty()) {
		...
		if(casp.delaydiv <= 0){	
				currCell.age ++  ; // increase age of cells
		}

		d_prob = (double)rand()/(double)RAND_MAX;

	   	if(d_prob <= casp.spdeath) //spontaneous death
	   	{
			//printf("d_prob is %lf\n", d_prob);
		...
		else
		{
			...
			// Instead of returning number of neighbours we will give the local density
			Ae = host.a_intm[currCell.place+casp.indcNeigh[0]]+
				 host.a_intm[currCell.place+casp.indcNeigh[1]]+
				 host.a_intm[currCell.place+casp.indcNeigh[2]]+
				 host.a_intm[currCell.place+casp.indcNeigh[3]]+
				 host.a_intm[currCell.place+casp.indcNeigh[4]]+
				 host.a_intm[currCell.place+casp.indcNeigh[5]];// /SVOL;
...
//             		currCell.is_alive = true     ;
			...
			//migrate	
           	}else if(Ae>=casp.adhes && currCell.is_alive && casp.phenchange > 0){ //Currently not in use. Phenchange is always <=0
                ...
                if(newPos){
                	...
                	cellsTmp.push_back(currCell);
            //migrate
			}else if(Ae>=casp.adhes && currCell.is_alive && casp.phenchange <=0 && !casp.phen[currCell.place] && nPhen == 0){ 
				//in use. !casp.phen[currCell.place] && nPhen == 0 means cell has migratory phenotype and neighbours have also migratory phenotype.
				newPos = returnPosChance(casp,host,id,currCell.place);
      			if(newPos){
