...
void hostInitialize(DataArray& host)
{
	...
	host.a_first = (double*) malloc(size);
	...
	if (host.a_first     == NULL) Error("Host.a_first memory allocation failed\n");
#endif
	...
	char filename[4096];
	sprintf(filename, "IC/new_dens_%s_D5.raw", host.dat);
	...
	memcpy(host.a_first,host.a,size);
    ...
    if (bIC!=NULL)
    {
        printf("Loading Initial Conditions for F on Day 5 ... \r\n");
        //fread(host.ecm,sizeof(double),total,ff);
    }else
    {
        printf("Initial Conditions for ECM not found. \r\n");
