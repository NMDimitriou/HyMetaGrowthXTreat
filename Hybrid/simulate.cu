...
void simulate(DataArray& host, DataArray& device1, DataArray& device2, CAArray& casp, vector<Cell>& cells, vector<Cell>& cellsTmp, InitialData& id)
{
	...
	//int cv = 0 ; //index for viability interval
	...
	printf("dx,y,z = %f\n", id.dx);
	printf("Da = %lf, s = %lf, k = %lf, Chi = %lf\n", id.da, id.s, id.ka, id.chi);
	printf("Db = %lf, r = %lf\n", id.db, id.r);
	printf("\n########################################## \n");
	printf("\n");
	...
	//-CA
	//scaleGrowth(id);
	casp.agediv     = log(2.0)/(id.s*id.dt_imp); //log(2.0)/(0.2*id.dt_imp);
	casp.spdeath    = id.ka*id.dt_imp; //spontaneous death probability
    //double viability[4] = {0.98323,0.971167,0.9671,0.98267};
	...
	int exportInterval[9] = {//int(1.0f/id.dt_imp) ,int(2.0f/id.dt_imp) ,int(3.0f/id.dt_imp) ,int(4.0f/id.dt_imp),
                              ///int(5.0f/id.dt_imp) ,
							  int(6.0f/id.dt_imp) ,int(7.0f/id.dt_imp) ,int(8.0f/id.dt_imp),
                              int(9.0f/id.dt_imp) ,int(10.0f/id.dt_imp),int(11.0f/id.dt_imp),int(12.0f/id.dt_imp),
     						  int(13.0f/id.dt_imp),int(14.0f/id.dt_imp)};

	//int viabInterval[4]   = {int(10.0f/id.dt_imp),int(11.0f/id.dt_imp),int(12.0f/id.dt_imp),int(13.0f/id.dt_imp)};
	int start_tp          = int(5.0f/id.dt_imp);

	// Export Day 0.
	ExportCA(fca,id,cells,start_tp);
	ExportCellCount(fna,id,cells,start_tp);
	...
	for(int i=start_tp; i<=id.itmax; i++)
	{
    	//Strang splitting
		//EXPLICIT STEP
		...
		if((i-exportInterval[c]) > - 1.0e-6)
		{
			...
			if(casp.adhes >=5) {casp.adhes=5;}
           	casp.delaydiv --;
            casp.phenchange --;
			//if((i-viabInterval[cv]) > - 1.0e-6)
            //{
            //    printf("Day %d\n", viabInterval[cv]);
            //    casp.spdeath = (1.0 - double(viability[cv]))*id.dt_imp;
            //    printf("Spontaneous death probability is %lf\n", casp.spdeath);
            //    cv++;
            //}
