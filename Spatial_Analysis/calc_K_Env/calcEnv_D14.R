...
calcEnv <- function(A,fname){
  ...
  calcEnv <- envelope.pp3(coorA, K3est, correction = c("translation", "isotropic"), nsim=99, nrank=1)#, transform = expression(. -4*pi*r^3/3))
  write.csv(calcEnv,paste("../resEnv_series_2_sim_comp/Kenv_raw_series_2_all_corrections",fname,"csv",sep = "."),row.names = FALSE)
  return(fname)
}

dat.files     <- list.files(path="../res_coord_series_2_scaled/D14",recursive=T,pattern="*.txt",full.names=T) # Must be scaled coordinates!!!
