...
calcEnv <- function(A,fname){
  ...
  #calcEnv <- envelope.pp3(coorA, K3est, correction = c("isotropic"), nsim=99, nrank=1)#, transform = expression(. -4*pi*r^3/3))
  calcEnv <- K3est(coorA, correction = c("isotropic"))
  write.csv(calcEnv,paste("../resEnv_CA_run3_RADIAL_TREAT_HYBRID_NO_R/Kenv",fname,"csv",sep = "."),row.names = FALSE)
  return(fname)
}

# Series 12
dat.files     <- list.files(path="../run3_RADIAL_TREAT_HYBRID_NO_R_adhes-5_phenchange0/scale_coord/",recursive=T,pattern="*.txt",full.names=T) # Must be scaled coordinates!!!
